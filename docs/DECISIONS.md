...
One entry per design decision point: what was chosen, what it was chosen over, and the
consequence. Decisions that carry a non-obvious tradeoff have the reasoning written out;
the rest are recorded factually because the alternative was simply worse.
...
- Reliable classes (1,2) share one per-peer counter; class 0 has its own dedup-only
  counter, excluded from cum_ack/SACK/ring/retransmit. (Clarifies DESIGN.md §5.2; see
...
- Rationale: approved 2026-07-27 in the tautq protocol brief (§0) - chaos scenarios
